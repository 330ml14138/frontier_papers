const DATA_PATHS = [
  "standalone-urban-marriage-tracker/data/urban_marriage_tracker.json",
  "./standalone-urban-marriage-tracker/data/urban_marriage_tracker.json",
  "/frontier_papers/standalone-urban-marriage-tracker/data/urban_marriage_tracker.json",
];

const TRACKER_KEY = "urban_marriage_tracker";
...
function journalMatchesFilter(journal) {
  ...
  const selectedField = els.fieldFilter.value;
  if (selectedJournal && journal.name !== selectedJournal) {
    return false;
  }
  if (selectedField && journal.field !== selectedField) {
    return false;
  }
  return true;
}
...
function renderSummary(journals) {
  ...
  const fieldCount = new Set(journals.map((j) => j.field).filter(Boolean)).size;
  ...
  els.summaryCards.innerHTML = `
    ...
    <article class="summary-card">
      <div class="summary-label">学科/方向</div>
      ...
      <section class="journal-card">
        <div class="journal-header">
          ...
          <div class="journal-meta">
            ${escapeHtml(journal.field || "未分类")} · 当前期次：<a href="${escapeHtml(journal.issue_url || "#")}" target="_blank" rel="noopener">${escapeHtml(journal.issue_title || "Latest issue")}</a>
...
function fillFilterOptions() {
  ...
  const fields = [...new Set(state.journals.map((journal) => journal.field).filter(Boolean))].sort();
  els.fieldFilter.innerHTML = `<option value="">全部学科</option>`;
  for (const field of fields) {
    els.fieldFilter.insertAdjacentHTML("beforeend", `<option value="${escapeHtml(field)}">${escapeHtml(field)}</option>`);
  }
...
async function loadData(showLoadingText = true) {
  if (window.location.protocol === "file:") {
    ...
    showError("请用 `python -m http.server 8000` 后访问 `http://localhost:8000/urban-realestate-marriage.html`。");
    return;
  }
  ...
  try {
    ...
    const tracker = payload[TRACKER_KEY] || {};
    ...
    const updatedAt = payload.updated_at ? `${payload.updated_at} UTC` : "尚未生成";
...
function bindEvents() {
  ...
  els.fieldFilter.addEventListener("change", renderJournals);
